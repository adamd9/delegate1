...
- When the task finishes, you'll receive a brief notification (prefixed with [COPILOT TASK NOTIFICATION]) containing the exact task ID, task link, and recorded delivery preference. The durable task runner automatically delivers the result through that preference, so do not fetch and resend it.
- Only intervene when the user asks for more detail, the task needs user input, or delivery reports a terminal failure. Use \`copilot_task_status\` with the exact task ID when you need the full output.
